...
node
{
	...
	try
	{
		...
		//		"""
			...
			bat """
				cd /D \"${engineLocalPath}"
				Engine/Build/BatchFiles/RunUAT.bat BuildGraph -Script=Engine/Build/Graph/Examples/BuildEditorAndTools.xml -Target="Submit To Perforce for UGS" "${additionalBuildGraphOption}" -set:EditorTarget="${projectTargetName}Editor" -set:ArchiveStream=//streamsDepot/ue5-Binaries
				"""
		}

		stage( 'Move to binaries workspace' )
		{
				// As move command does want to overwrite the read only file, go through multiple command
			bat """
				cd /D \"${engineLocalPath}\\"
				...
				"""
		/*
			// add "exit /b" as robocop return 1 on success
			bat """
				cd /D \"${engineLocalPath}\\"
				robocopy "LocalBuilds\\ArchiveForUGS\\Perforce" "${binariesZipLocalFolder}" Unknown-${projectTargetName}Editor.zip /E /MOVE
				rem // This handles the exit code (`ErrorLevel`) returned by `robocopy` properly:
				if ErrorLevel 8 (exit /B 1) else (exit /B 0)
				del /f "${binariesZipLocalFolder}\\${binariesZipFileName}"
				IF NOT ERRORLEVEL 1 ECHO delete successful
				ren "${binariesZipLocalFolder}\\Unknown-${projectTargetName}Editor.zip" "${binariesZipFileName}"
				IF NOT ERRORLEVEL 1 ECHO rename successful
				del /f "${binariesZipLocalFolder}\\Unknown-${projectTargetName}Editor.zip"
				IF NOT ERRORLEVEL 1 ECHO delete successful
				"""
		*/
		}
		...
		stage( 'Submit' )
		{
			...
			p4publish credential: perforceCredentialInJenkins,
				publish: submit(delete: false, description: "[CL ${clNumberAsDigit}] ${env.JOB_NAME} ${env.BUILD_NUMBER} ${optionFullRebuild?'rebuild':''}".toString(), onlyOnSuccess: false, purge: '', reopen: false),
