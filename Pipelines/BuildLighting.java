...
// LIGHTING_QUALITY: Choice: Preview/Medium/High/Production
//
// Precondition:
...
node
{
	try
	{
		...
		def lightingQuality = LIGHTING_QUALITY
		...
		stage( 'Command' )
		{
			...
			/*def result = bat """
				...
				UE4Editor-Cmd.exe \"${projectLocalPath}\\${projectName}.uproject\" ${mapName} -AutomatedMapBuild -buildlighting -quality=${lightingQuality} UseSCC=false -unattended -buildmachine
				"""*/
			def result = bat """
				cd /D \"${engineLocalPath}\\Engine\\Binaries\\Win64\"
		...
		stage('Submit')
		{
			...
			def submittedFiles = p4.run('submit', '-d', '[jenkins] Lighting build')
